...
const staticServer = new nodeStaticAlias.Server(WEB_DIR, {
...
function serveFile(url, statusCode, headers, req, res) {
	const listener = staticServer.serveFile(url, statusCode, headers, req, res);
...
async function getPostIDs() {
	const files = await fsReadDir(path.join(WEB_DIR, "posts"));
	return (
		files
